...
router.post('/', async (req, res) => {
    ...
    if (correctPassword) {
        ...
        if (!user.firstLogin) {
            res.cookie('token', token, {maxAge: 24 * 60 * 60 * 1000, httpOnly: true, secure: true, sameSite:'none' }); //maxAge is in miliseconds???? why???? pero ayan 1 day
    ...
    crypto.randomBytes(32, (err, buffer) => {//creating reset token
        ...
        .then(user => {
            ...
            return user.save().then(result => {
                ...
                let resetLink = token;
                let body = email.RP1 + user.firstName + " " + user.lastName + "," + email.RP2 + '<a href="' + process.env.RESET + token + '">this link </a>' + email.RP3; 
...
try {
    res.cookie('token', "null", {maxAge: 30, httpOnly: true, secure: true, sameSite:'none' });
    res.clearCookie('token', {path:'/',domain:'tmis-backend.herokuapp.com'});
    //res.redirect('/')
